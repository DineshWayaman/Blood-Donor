...
$bloogGroup = $_GET['bgroup'];

if ($bloogGroup=='Opositive') {
    $group = 'O+';
}elseif($bloogGroup=='Om'){
    $group = 'O-';
}elseif($bloogGroup=='Ap'){
    $group = 'A+';
}elseif($bloogGroup=='Am'){
    $group = 'A-';
}elseif($bloogGroup=='ABp'){
    $group = 'AB+';
}elseif($bloogGroup=='ABm'){
    $group = 'AB-';
}
else{
    $group = 'Sorry. No donors Fond Your Aria..';
}

$searchDonors = "SELECT * FROM `users` WHERE `u_blood_group`='$group' AND `u_status` ='1'";
            ...
            <div class="main-donor-card shadow">
                ...
                <?php 
                    if ($searchuserfetch['u_facebook_link']==null) {
                        ...
                        ?>
                <a href="<?php echo $searchuserfetch['u_facebook_link'];  ?>" target="_blank"><p class="text-center read-hover m-0" style="font-size: 18px;">Facebook</p></a>
                        <?php
                    }
                        ?>
